...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *pjvm, void *reserved) {
    javaVm = pjvm;
    JNIEnv * env = getEnv();
    jclass randomClass = env->FindClass("org/radarcns/opensmile/SmileJNI");
...
JNIEXPORT jstring JNICALL Java_org_radarcns_opensmile_SmileJNI_SMILExtractJNI( JNIEnv* env, jobject thiz, jstring conf, jint updateProfile,jstring output){
    ...
    const char *cmd[] = {SMILExtract, "-C", configfile, "-O", outputfile, "-nologfile", "-l", "2"};
    // TODO: sdcard location, if needed: Environment.getExternalStorageDirectory()
    int cmd_num = 8; // IMPORTANT: synchronize with the size of the cmd array
