...
import static org.radarcns.android.DeviceStatusListener.Status.CONNECTED;
import static org.radarcns.android.DeviceStatusListener.Status.CONNECTING;
import static org.radarcns.android.DeviceStatusListener.Status.DISCONNECTED;
import static org.radarcns.android.DeviceStatusListener.Status.READY;
...
public class OverviewActivity extends AppCompatActivity {
    protected final static Logger logger = LoggerFactory.getLogger(OverviewActivity.class);
        ...
        public void onReceive(Context context, final Intent intent) {
            if (intent.getAction().equals(DEVICE_CONNECT_FAILED)) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(OverviewActivity.this, "Cannot connect to device " + intent.getStringExtra(DEVICE_STATUS_NAME), Toast.LENGTH_SHORT).show();
                    }
                });
            }
        }
    ...
    public void deviceStatusUpdated(final E4ServiceConnection connection, final DeviceStatusListener.Status status) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(OverviewActivity.this, status.toString(), Toast.LENGTH_SHORT).show();
                switch (status) {
                    case CONNECTED:
//                        addDeviceButton(connection);

                        synchronized (OverviewActivity.this) {
                            if (activeConnection == null) {
                                activeConnection = connection;
                            }
                        }
